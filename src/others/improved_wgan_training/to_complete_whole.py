...
input_dir = "../../data/babara/testInpainting/corrupted_patches"
#output_dir = "./reconDir"
output_dir = "../../data/babara/testInpainting/recon_patches"
lam = 0.1
zlen = 100
ITER = 5000
momentum = 0.
...
with tf.Session() as sess:
    saver = tf.train.import_meta_graph("./checkpoint/toy_image.model-4950.meta")
    saver.restore(sess, "./checkpoint/toy_image.model-4950")
    ...
    contextual_loss = tf.reduce_mean(
            tf.reduce_sum(tf.abs(tf.multiply(mask, gen_imgs) - real)))
    ...
    for idx in xrange(0, batch_idxs):
        ...
        patch_names = names[idx * batch_size:(idx+ 1) * batch_size]
        realImgs = np.array([readImg(batch_file) for batch_file in batch_files])

        realscore = sess.run(real_score, feed_dict={real: realImgs})

        curmask = (realImgs > -0.99).astype(np.float32)

        zhats = stats.truncnorm.rvs((lower-mu)/sigma, (upper-mu)/sigma, loc=mu, scale=sigma, size=(len(batch_files),zlen))
        v = 0
        for i in range(ITER):
            runs = [complete_loss, grad_complete_loss,fake_score]
            fd = {real: realImgs, z: zhats, mask:curmask}
        ...
        for i in range(samples.shape[0]):
            saveImg(recover[i], os.path.join(output_dir, patch_names[i] + ".png"))


img = reconstruct_from_patches_2d(output_dir, (256, 256, 3))
saveImg(img, os.path.join("../../data/babara/testInpainting", "recon.png"))
"""
